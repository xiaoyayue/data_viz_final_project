...
The total5 dataset stores information largely similar to total but is instead a timeseries data. This dataset is also the result of merging multiple gapminder datasets in RStudio and is used to show the general trend of life expectancy and compared its match to that of economic growth and ECI. 
...
The global_power_plant_database is used to produce the world map visualization of power plants number, location, and primary fuel type in each country. The dataset is obtained from the World Resource Institute. 
...
With the help of the power plant visualization above, however, we can find that countries or regions that have high percentage usually have more power plants whose primary fuel type are renewable energies. For example, these two indications co-occur in many Northern European countries, South American countries, and Southeastern countries. Although we cannot have any definitive conclusion, it directs policy makers to an areas worth studying in the future. 
